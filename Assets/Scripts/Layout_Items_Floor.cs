...
public class Layout_Items_Floor : MonoBehaviour
{
    private Vector2 _playerFingerPs;

    void Start()
    {
        //ゲームスタート時から存在していないとダメ
    }
    ...
    {
        if (other.gameObject.tag == "Layout_Collision_Tile")
        {
            this.gameObject.GetComponent<SpriteRenderer>().color = new Color32(255, 0, 0, 150);

            //決定ボタンを無効化
            GameObject.FindWithTag("Layout_Dicide_Btn").GetComponent<Button>().interactable = false;
