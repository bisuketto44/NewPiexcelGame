...
public class ChatSortNumber : UIBehaviour
{
    ...
    public void HiperChatContentDesplay()
    {
        ...
        var SortList = GameObject.FindWithTag("HiperChatGanarates").GetComponent<Hiper_Chat_Generates>();

        //格納されたcount変数を使ってリスト内のチャットの値段と、保存したコメント内容を読んでくる
        MoneyText.text = SortList.SortList[ChatNumber].value.ToString("N0");
        ChatContentText.text = SortList.SortList[ChatNumber].ChatContent;

        //スパチャの色に合わせてカラーを決定
        switch (SortList.SortList[ChatNumber].number)
        {
            case 0:
                break;
            case 1:
        ...
        //ユーザーアイコンのカラーを決定
        switch (SortList.SortList[ChatNumber].No)
        {
            case 0:
    ...
    //表示内容の変更(何番目を表示するかでソートしたリストから持ってくる)
    public void UpdateChat(int count)
    {
        //ソートした後のリストの一番目を取得してくる
        var SortList = GameObject.FindWithTag("HiperChatGanarates").GetComponent<Hiper_Chat_Generates>();

        //値段を表示
        ValueText.text = SortList.SortList[count].value.ToString("N0");

        //値段に合わせて色を変更
        switch (SortList.SortList[count].number)
        {
            case 0:
